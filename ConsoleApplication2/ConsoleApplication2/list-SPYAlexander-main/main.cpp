...
void TestSortedList()
{
    ...
    cout << "= = = = = = = = = = = = = = = = = = = = = " << endl;
    cout << endl
         << "F i n i s h e d   S u c c e s s f u l l y" << endl;
}
...
void GradingTest(List *list)
{
    ...
    if (list->Insert(person, 0) == true)
    {
        ++passed;
    }
    if (list->Get(0)->ToString() == "Person: {name: George, age: 12}")
    {
        ++passed;
    }
    if (list->Size() == 1)
    ...
    if (list->IndexOf(GEORGE) == 0)
    {
        ++passed;
    }

    integer = new Integer(10);
    if (list->Insert(integer, 0) == true)
    {
        ++passed;
    }
    if (list->Get(0)->ToString() == "10")
    {
        ++passed;
    }
    if (list->Get(1)->ToString() == "Person: {name: George, age: 12}")
    {
        ++passed;
    }
    if (list->Size() == 2)
    ...
    if (list->IndexOf(TEN) == 0)
    {
        ++passed;
    }
    if (list->IndexOf(GEORGE) == 1)
    {
        ++passed;
    }
    if (list->IndexOf(DUMMY) == -1)
    {
        ++passed;
    }

    person = new Person("Patrick", 1);
    if (list->Insert(person, 1) == true)
    {
        ++passed;
    }
    if (list->Size() == 3)
    ...
    if (list->Insert(new Person("Violet", 1), list->Size()) == true)
    {
        ++passed;
    }
    if (list->Size() == 6)
    ...
    if ((object = list->Remove(5)) != nullptr)
    {
        ++passed;
    }
    if (object->ToString() == "Person: {name: Violet, age: 1}")
    {
        ++passed;
    }
    // Fixing the Leak
    ...
    {
        inputData >> name >> age >> position;
        if (list->Insert(new Person(name, age), position) == true)
        {
            ++passed;
        }
        inputData >> number >> position;
        if (list->Insert(new Integer(number), position) == true)
        {
            ++passed;
        }
    }
    ...
    for (size_t i = 0; i < 5; i++)
    {
        insertedData >> name >> age >> position;
        object = new Person(name, age);
        if (list->IndexOf(object) == static_cast<int>(position))
        {
            ++passed;
        }
        insertedData >> number >> position;
    ...
    if ((object = list->Remove(4)) != nullptr)
    {
        ++passed;
    }
    if (list->Size() == 14)
    ...
    if ((object = list->Get(7)) != nullptr)
    {
        ++passed;
    }
    if (object->ToString() == "Person: {name: Peter, age: 43}")
    {
        ++passed;
    }
